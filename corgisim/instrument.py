...
from corgisim.sat_spots import add_cos_pattern_dm, add_custom_pattern_dm
import cgisim
import corgisim
...
class CorgiOptics():
    ...
    def __init__(self, cgi_mode = None, bandpass= None,  diam = 236.3114, optics_keywords=None, roll_angle= 0, satspot_keywords=None, stellar_diam_and_jitter_keywords=None, oversampling_factor = 7, return_oversample = False, **kwargs):
        '''
        ...
        Args:
            ...
            **kwargs:
            ...
            - visit_id (str, optional): A string indicating the visit ID for populating header VISITID.
        ...
        self.visit_id = kwargs.get("visit_id", "0200001001001001001")
        ...
        self.SATSPOTS = 0
        # provenance of a custom satspot DM pattern (Alternate Probe support), populated by add_satspot
        self.satspot_info = None
    ...
    def get_host_star_psf(self, input_scene, sim_scene=None, on_the_fly=False):
        ...
        # custom-pattern (Alternate Probe) provenance: pattern name, scale, sign
        if getattr(self, 'satspot_info', None):
            sim_info.update(self.satspot_info)
    ...
    def add_satspot(self,satspot_keywords):
        ...
        1. Analytical satellite spots (default): `satspot_keywords` contains
           'num_pairs', 'sep_lamD', 'angle_deg', 'contrast' (and optionally
           'wavelength_m', 'sign'); a cosine pattern is generated by
           sat_spots.add_cos_pattern_dm.
        ...
        2. Custom DM pattern (Alternate Probe support): `satspot_keywords`
           ...
           keywords must not be mixed with 'custom_pattern'.
        ...
        if satspot_keywords == None:
            self.SATSPOTS = int(0)
        elif 'custom_pattern' in satspot_keywords:
            # Custom externally supplied DM pattern (e.g., Gaussian probes for the Alternate Probe experiment)
            if self.optics_keywords['use_dm1'] != 1:
                raise KeyError('ERROR: use_dm1 in optics_keywords is not set 1')

            analytic_keys = {'num_pairs', 'sep_lamD', 'angle_deg', 'contrast', 'wavelength_m'}
            mixed_keys = analytic_keys & satspot_keywords.keys()
            if mixed_keys:
                raise KeyError(f"ERROR: 'custom_pattern' cannot be combined with analytical satspot keywords: {mixed_keys}")
            ...
            sign = satspot_keywords.get('sign', 'positive')
            scale = satspot_keywords['scale']

            self.optics_keywords['dm1_v'] = add_custom_pattern_dm(
                self.optics_keywords['dm1_v'],
                satspot_keywords['custom_pattern'],
                scale,
                sign=sign,
            )

            # Provenance recorded as COMMENT headers in the simulated products
            # (and therefore in the L1 primary header), so every output file
            # self-documents which probe pattern, amplitude scale, and sign
            # were applied.
            self.satspot_info = {
                'satspot_pattern_name': satspot_keywords.get('pattern_name', 'user-supplied custom pattern'),
                'satspot_scale': scale,
                'satspot_sign': sign,
            }
    ...
    def remove_satspot(self, satspot_keywords):
        ...
        # Update header and clear custom-pattern provenance
        self.SATSPOTS = int(0)
        self.satspot_info = None
    ...
    def simulate_2d_scene(self, input_scene, prf_cube_path, sim_scene=None, interpolate_prfs=False):
        ...
        positions = conv.get_valid_polar_positions(
            radii_lamD,
            azimuths_deg,
        )
    ...
    def generate_detector_image(self, simulated_scene, exptime, full_frame= False, cut_sub_frame=False,  loc_x=512, loc_y=512):
        ...
        if full_frame:
            ...
            header_info = {'EXPTIME': exptime,'EMGAIN_C':self.emccd_keywords_default['em_gain'],'PSFREF':ref_flag,
                           ...
                           'EACQ_ROW': loc_y, 'EACQ_COL': loc_x,'RN': self.emccd_keywords_default['read_noise'],'target_name': sim_info['target_name'],'VISTYPE': sim_info['visit_type'],'VISITID': sim_info['visit_id'],'RA':sim_info['RA'],'DEC':sim_info['DEC']}
