...
def get_slit_mask(config, dx_hires_um = 0.1, hires_dim_um = 800, binfac = 50):
    ...
    slit_ref_params = read_slit_params(config.slit_param_fname)
    if config.slit not in slit_ref_params.keys():
        raise Exception('ERROR: Requested slit {:s} is not defined in {:s}'.format(config.slit, config.slit_param_fname))
...
def apply_prism(config, image_cube):
    """
    ...
    Parameters
    ----------
    config : CorgiOptics configuration object
        object containing:
    ...
    2. Creates a densely sampled wavelength grid based on config.wav_step_um
    ...
    """
    prism_params = read_prism_params(config.prism_param_fname)
    theta = prism_params['clocking_angle']

    dispersed_slices = []
    # Define a densely sampled wavelength array for the cube of dispersed images
    N_wav_interp = int(round((config.lam_um[-1] - config.lam_um[0]) / config.wav_step_um)) 
    
    dispersion_polyfunc = np.poly1d(prism_params['pos_vs_wavlen_polycoeff'])
    interp_wavs_bandpass = np.linspace(config.lam_um[0], config.lam_um[-1], N_wav_interp)
    delta_wavelen = interp_wavs_bandpass - config.lamref_um

    model_sampling_mm = config.sampling_um / config.oversampling_factor * 1E-3
    dispers_shift_mm = dispersion_polyfunc(delta_wavelen / config.lamref_um)
    ...
    cube_grid = (config.lam_um, y_pts, x_pts)
    
    #Make interp_wavs_grid
    ...
    # center_lam_um = config.lam0_um
    # delta_wavelen_filter_center = center_lam_um - config.lamref_um
    # disp_shift_filter_center_modelpix = dispersion_polyfunc(delta_wavelen_filter_center / config.lamref_um) / model_sampling_mm
    ...
    # xcent = (xcent_ovsamp + 0.5) / config.oversampling_factor - 0.5
    # ycent = (ycent_ovsamp + 0.5) / config.oversampling_factor - 0.5

    # fsam_image = np.zeros((dispersed_cube.shape[0],
                        #    dispersed_cube.shape[1] // config.oversampling_factor,
                        #    dispersed_cube.shape[2] // config.oversampling_factor))
    # for ww in range(fsam_image.shape[0]):
        # fsam_image[ww,:,:] = (dispersed_cube[ww,:,:].reshape(
                                # (fsam_image.shape[1], config.oversampling_factor,
                                # fsam_image.shape[2], config.oversampling_factor)).mean(3).mean(1) *
